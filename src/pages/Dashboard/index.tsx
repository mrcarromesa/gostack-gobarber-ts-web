...
const Dashboard: React.FC = () => {
  ...
  useEffect(() => {
    const loadMonthAvailability = async (): Promise<void> => {
  ...
  useEffect(() => {
    const loadAppointments = async (): Promise<void> => {
      const { data } = await api.get<Appointments[]>('appointments/me', {
        params: {
      ...
      const appointmentsFormatted = data.map((appointement) => ({
        ...appointement,
        hourFormatted: format(parseISO(appointement.date), 'HH:mm'),
      }));
  ...
  const disabledDays = useMemo(() => monthAVailability
  ...
  const mornigAppointments = useMemo(() => appointments
    .filter((appointment) => parseISO(appointment.date).getHours() < 12), [appointments]);
  const afternoonAppointments = useMemo(() => appointments
    .filter((appointment) => parseISO(appointment.date).getHours() >= 12), [appointments]);

  const nextAppointment = useMemo(() => appointments
    .find((appointment) => isAfter(parseISO(appointment.date), new Date())), [appointments]);
